...
class WindowsWrapper(torch.nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        resized_global_tensor = F_vision.resize(tensors, size=(window_h, window_w))
        ...
        # )
        concat_tensors = torch.cat(
            [F_vision.resize(global_features[0].tensors, size=window_tensors.shape[-2:]), window_tensors], dim=1
        )
